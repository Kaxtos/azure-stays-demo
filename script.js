...
function renderOverview(){
  ...
  <div class="twocol"><article class="panel"><small>REVENUE PERFORMANCE · ${esc(p.name.toUpperCase())}</small><h3>${money(ytd)} <span>this year</span></h3><div class="chart">${vals.map((v,i)=>`<i style="height:${Math.max(3,Math.round(v/max*95))}%" title="${MON[i+3]}: ${money(v)}"></i>`).join('')}</div><div class="months m6">${months.map(m=>`<span>${MON[+m.slice(5)-1].toUpperCase()}</span>`).join('')}</div></article>
